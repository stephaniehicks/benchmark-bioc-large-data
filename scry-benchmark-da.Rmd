...
```{r}
time_start <- proc.time()
tmp <- nullResiduals(tenx, assay="counts", 
                     fam = "poisson", type="deviance")
...
multicoreParam <- MulticoreParam(workers = 1)
bpparam()
```
...
```{r}
null_realized <- function(object, assay, fam, type, rank){
    ...
    object <- nullResiduals(object, assay=assay,
                                  fam = fam, type=type)
    assays(object, "PCA", withDimnames = TRUE) <- 
      runPCA(assay(object, .id), rank=rank, BSPARAM=IrlbaParam())
    object
}

tmp <- null_realized(tenx_realized, assay= "counts", 
                fam = "poisson", type = "deviance", rank=10)
dim(reducedDim(object, "PCA"))
